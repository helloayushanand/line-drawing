...
class CoordinateExtractor:
    ...
    def extract_lines_from_generated_image(
        self,
        ...
        """
        Extract line coordinates from generated image using CV methods only
        """
        # Method 1: Image diff + Hough Transform (most reliable)
        diff_lines = self._extract_via_diff(generated_image, original_image)
        
        # Method 2: Direct CV detection on generated image
        cv_lines = self._extract_via_cv(generated_image)
        
        # Merge and refine (no AI needed for simple lines)
        merged_lines = self._merge_lines_cv_only(diff_lines, cv_lines, original_image)
        
        # Filter lines outside product mask
        product_mask = self._detect_product_mask(original_image)
        filtered_lines = self._filter_lines_outside_product(merged_lines, product_mask, original_image)
        
        # Rank and select top N lines
        if expected_line_count:
            filtered_lines = self._rank_and_select_lines(filtered_lines, expected_line_count, original_image, product_mask)
    ...
    def _extract_via_diff(
        self,
        generated: Image.Image,
        original: Image.Image
    ) -> List[dict]:
        """Extract lines by comparing generated vs original"""
        # Resize generated image to match original size
        orig_size = original.size  # (width, height)
        if generated.size != orig_size:
            generated = generated.resize(orig_size, Image.Resampling.LANCZOS)
        
        gen_arr = pil_to_numpy(generated)
        orig_arr = pil_to_numpy(original)
        
        # Calculate difference
        diff = cv2.absdiff(gen_arr, orig_arr)
        diff_gray = cv2.cvtColor(diff, cv2.COLOR_RGB2GRAY)
        
        # Threshold to get only the drawn lines
        _, thresh = cv2.threshold(diff_gray, 30, 255, cv2.THRESH_BINARY)
        
        # Detect lines in the diff
        lines = detect_lines_hough(thresh)
        
        # Convert to normalized coordinates (use original image size)
        h, w = orig_arr.shape[:2]
        normalized_lines = []
        for x1, y1, x2, y2 in lines:
            # Calculate confidence based on diff strength along the line
            # Sample points along line and get average diff value
            num_samples = 10
            diff_values = []
            for i in range(num_samples + 1):
                ...
                y = int(y1 * (1 - t) + y2 * t)
                if 0 <= y < h and 0 <= x < w:
                    diff_values.append(diff_gray[y, x])
            
            avg_diff = np.mean(diff_values) if diff_values else 0
            # Normalize to 0-1 (assuming max diff is 255)
            confidence = min(1.0, avg_diff / 255.0)
    ...
    def _merge_lines_cv_only(
        self,
        diff_lines: List[dict],
        cv_lines: List[dict],
        original_image: Image.Image
    ) -> List[dict]:
        """Merge lines from CV methods, prioritize AI labels"""
        # Use CV lines as base (they have labels)
        if not cv_lines:
            # Fallback to diff lines
            cv_lines = diff_lines
        ...
        for cv_line in cv_lines:
            # Find closest matching line from diff or CV
            best_match = None
            min_dist = float('inf')
            
            all_cv_lines = diff_lines + cv_lines
            for cv_line_other in all_cv_lines:
                # Calculate distance between line midpoints
                cv_mid = (
                    (cv_line["start"]["x"] + cv_line["end"]["x"]) / 2,
                    (cv_line["start"]["y"] + cv_line["end"]["y"]) / 2
                )
                cv_mid_other = (
                    (cv_line_other["start"]["x"] + cv_line_other["end"]["x"]) / 2,
                    (cv_line_other["start"]["y"] + cv_line_other["end"]["y"]) / 2
                )
                dist = np.sqrt(
                    (cv_mid[0] - cv_mid_other[0])**2 + 
                    (cv_mid[1] - cv_mid_other[1])**2
                )
                
                if dist < min_dist and dist < settings.FEATURE_MATCH_THRESHOLD:
                    min_dist = dist
                    best_match = cv_line_other
            
            # Use CV line with refined coordinates from CV if available
            if best_match:
                # Refine coordinates using edge detection
                refined = self._refine_with_edges(cv_line, original_image)
                merged.append(refined)
            else:
                # Still refine with edges
                refined = self._refine_with_edges(cv_line, original_image)
                merged.append(refined)
